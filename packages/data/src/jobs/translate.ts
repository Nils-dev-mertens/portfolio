/**
 * Generates the `_en` translation columns from the Dutch source content using
 * DeepL. Intended to run after `db:seed` (or on demand). It ONLY fills `_en`
 * columns that are currently empty, so any manual English edits are preserved.
 *
...
import { about, projects, work_experience, education } from '../db/schema';

interface Entry {
  table: 'about' | 'projects' | 'work_experience' | 'education';
  id: string;
...
export async function runTranslate() {
  ...
  for (const e of entries) {
    const row =
      e.table === 'about'
        ? db.select().from(about).limit(1).all()[0]
        : e.table === 'projects'
          ? db.select().from(projects).where(eq(projects.id, e.id)).all()[0]
          : e.table === 'work_experience'
            ? db.select().from(work_experience).where(eq(work_experience.id, e.id)).all()[0]
            : db.select().from(education).where(eq(education.id, e.id)).all()[0];
    if (!row) continue;
